...
export default function Projects () {
    ...
    useEffect(() => {
        axios.get("/project/list").then((res) => {
            ...
            setProjectList(specialProject.slice(0, 3));
            setAllProjects(specialProject);
            var futureProjects = [];
            if (config[network].featuredId) {
                futureProjects = res.data.filter((item) => item.project_uniq_id);
            } else {
                futureProjects = res.data.filter(
                    (item) =>
    ...
    const columns = [
        ...
        },
        {
            ...
            key: "status",
            render: (props) => (
                <span>{statusMapping[props] ? statusMapping[props] : props}</span>
            ),
        },
        {
            title: t("common.apy"),
            ...
            key: "expected_apy",
            render: (props) => <div className="apy">{props}%</div>,
        },
        {
            ...
            render: (props, record) => (
                <div>
                    {new Date(record.project_start_time).toLocaleDateString()} -{" "}
                    {new Date(record.project_end_time).toLocaleDateString()}
                </div>
            ),
        },
        {
            ...
            {featuredProject && (
                <div className="featured-project">
                    <Row>
                        <Col xs={24} md={11}>
                            <div className="featured-left">
                                <div>
                                    ...
                                    <div
                                        ...
                                        dangerouslySetInnerHTML={{
                                            __html: toBr(featuredProject.project_profile),
                                        }}
                                    ></div>
                                    {featuredCountdown > 0 && (
                                ...
                                <div>
                                    ...
                                    <Link
                                        className="join-btn"
                                        to={"/project/" + featuredProject.project_uniq_id}
                        ...
                        <Col xs={24} md={13}>
                            <img src={HomepageBanner} className="homepage-banner" alt="" />
                        </Col>
                    </Row>
                </div>
            )}
                ...
                <Row gutter={28}>
                    {projectList &&
                        projectList.map((item, index) => (
                            <Col xs={24} sm={12} md={24} lg={12} xl={8} key={index}>
                                <div className="project-item">
                                    ...
                                    {(item.status === "raising" ||
                                        item.status === "canInvest") && (
                                            <Progress
                                                ...
                                                status="active"
                                                percent={(
                                                    (item.current_raised_money / item.max_amount) *
                                                    100
                                                ).toFixed(0)}
                                                className="progress-bar"
                                            />
                                    ...
                                    <a
                                        className="join-btn"
                                        href={"/project/" + item.project_uniq_id}
                                        key={item.project_uniq_id}
                                    >
                                        {t("common.viewProject")}
                                    </a>
